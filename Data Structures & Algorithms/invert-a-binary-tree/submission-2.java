...
class Solution {
    ...
    public TreeNode invertTree(TreeNode root) {
        ...
        // invert the child of lst 
        TreeNode left = invertTree(root.left); // after inverting lst , return its node because for the root node which called  it , it will be left we have to swap that with the root right na 
        TreeNode right = invertTree(root.right); // after inverting rst(root.righ), return its node  because for the root node which called it , it will be right node so we have to swap that with the root's left na 
